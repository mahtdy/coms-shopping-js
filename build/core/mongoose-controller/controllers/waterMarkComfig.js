...
var waterMarkConfig = zod_1.z.object({
    ...
    lineSpacing: zod_1.z.number().min(0).optional(),
    wordSpacing: zod_1.z.number().min(0).optional(),
    position_x: zod_1.z.number().min(0),
    position_y: zod_1.z.number().min(0),
    x: zod_1.z.coerce.number().int().min(0).optional(),
    y: zod_1.z.coerce.number().int().min(0).optional(),
    transparency: zod_1.z.coerce.number().int().max(100).min(0).optional(),
    fontSize: zod_1.z.coerce.number().int().min(1).optional(),
    ...
    waterMarkSize: zod_1.z.number().min(0).optional(),
...
var insertSchema = zod_1.z.object({
    ...
    resultQuality: zod_1.z.coerce.number().int().max(100).min(0).optional(),
    resultSize: zod_1.z.coerce.number().int().optional(),
...
class WaterMarkController extends controller_1.default {
    ...
    async create(data, session) {
        ...
        try {
            ...
            img = await imageProccessing_1.default.refresh("src/uploads/tmp/", img, data);
            let imgs = img.split("/");
            let Dimention = await imageProccessing_1.default.calculateDimentions(img, 128, true);
            const demoImgSmall = await imageProccessing_1.default.resize("src/uploads/filters/demo/", img, "jpg", Dimention.x, Dimention.y);
            await fileManager_1.DiskFileManager.move(img, `src/uploads/filters/demo/`);
    ...
    async edit(id, watermark, session) {
        try {
            ...
            var img = session['img'];
            img = await imageProccessing_1.default.refresh("src/uploads/tmp/", img, watermark);
            let imgs = img.split("/");
            let Dimention = await imageProccessing_1.default.calculateDimentions(img, 128, true);
            const demoImgSmall = await imageProccessing_1.default.resize("src/uploads/filters/demo/", img, "jpg", Dimention.x, Dimention.y);
            await fileManager_1.DiskFileManager.move(img, `src/uploads/filters/demo/`);
            ...
            try {
                await fileManager_1.DiskFileManager.removeFile("src" + (exWatermark === null || exWatermark === void 0 ? void 0 : exWatermark.demoImg));
                await fileManager_1.DiskFileManager.removeFile("src" + (exWatermark === null || exWatermark === void 0 ? void 0 : exWatermark.demoImgSmall));
            }
            catch (error) {
            }
            return {
