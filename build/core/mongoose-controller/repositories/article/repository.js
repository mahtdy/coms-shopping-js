        ...
        if (doc.videos != undefined) {
            for (let i = 0; i < doc.videos.length; i++) {
                if (typeof doc.videos[i] != "string") {
                    files.push(doc.videos[i].src);
                    try {
                        for (let j = 0; j < doc.videos[i].result.length; j++) {
                            files.push(doc.videos[i].result[j].path);
                        }
                        for (let j = 0; j < doc.videos[i].screenshots.length; j++) {
                            files.push(doc.videos[i].screenshots[j].path);
                        }
                    }
                    catch (error) {
                    }
